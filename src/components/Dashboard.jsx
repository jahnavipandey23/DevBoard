...
export default function Dashboard({ setActivePage }) {
  const { user, kanbanProjects, stats, loading } = useGithub();
              ...
              <div key={bar.label} className="bar-group">
                <div className="bar-track">
                  <div className="bar-y-labels">
                    {[8, 6, 4, 2, 0].map(n => <span key={n} className="bar-y-label">{n}</span>)}
                  </div>
      ...
      <div className="card dash-recent" style={{ margin: "0 32px 32px" }}>
        ...
        {recent.length === 0 ? (
          ...
          <table className="recent-table">
            <thead>
              <tr><th>Project</th><th>Language</th><th>Priority</th><th>Status</th><th>Updated</th></tr>
            </thead>
            <tbody>
              {recent.map(p => (
                <tr key={p.id}>
                  <td>
                    <a href={p.url} target="_blank" rel="noopener noreferrer" className="project-link">{p.name}</a>
                  </td>
                  <td>
